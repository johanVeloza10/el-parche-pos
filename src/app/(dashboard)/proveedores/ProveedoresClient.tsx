...
export default function ProveedoresClient() {
  ...
  // Modal de Pago
  const [modalPagoId, setModalPagoId] = useState<string | null>(null);
  const [pagoFecha, setPagoFecha] = useState(new Date().toISOString().split("T")[0]);
  const [pagoComprobante, setPagoComprobante] = useState("");
  const [pagoLink, setPagoLink] = useState("");
  ...
  const confirmarPagoLiquidacion = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!modalPagoId) return;

    try {
      const res = await fetch(`/api/liquidaciones/${modalPagoId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ 
          estado: "PAGADA",
          fechaPago: pagoFecha,
          soportePago: JSON.stringify({
            fechaTransferencia: pagoFecha,
  ...
  return (
    <div className="flex flex-col gap-8 pb-16">
                    ...
                    {historialLiquidaciones.map(l => (
                      <div key={l.id} className="flex justify-between items-center p-3 rounded-xl bg-black/15 border border-zinc-800 text-xs font-sans">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {l.estado !== 'PAGADA' && (
                            <button 
                              onClick={() => handleRegistrarPago(l.id)}
                              className="p-2 rounded bg-[var(--color-primary)]/20 hover:bg-[var(--color-primary)]/40 text-[var(--color-primary)] transition-colors"
      ...
      {modalPagoId && (
        <div className="fixed inset-0 bg-black/90 flex items-center justify-center p-4 z-50 backdrop-blur-md">
          <div className="border-stitch-gold bg-[var(--color-surface)] rounded-3xl w-full max-w-md overflow-hidden shadow-2xl">
            <div className="p-6 border-b border-zinc-800 flex justify-between items-center bg-[var(--color-surface-elevated)]/50">
              <h2 className="text-xl font-bold text-white flex items-center gap-2 font-heading">
                <Landmark className="w-5 h-5 text-[var(--color-primary)]" /> Registrar Soporte de Pago
              </h2>
              <button onClick={() => setModalPagoId(null)} className="text-[var(--color-text-secondary)] hover:text-white p-1">
                <X className="w-6 h-6" />
              </button>
            </div>
            
            <form onSubmit={confirmarPagoLiquidacion} className="p-6 flex flex-col gap-4 font-sans text-sm">
              <div>
                <label className="block text-xs font-bold text-zinc-300 mb-1">Fecha de Transferencia *</label>
                <input 
                  type="date" required
                  className="w-full bg-[var(--color-surface-elevated)] border border-zinc-850 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-[var(--color-primary)]"
              ...
              <div>
                <label className="block text-xs font-bold text-zinc-300 mb-1">Nro. Comprobante Bancario *</label>
                <input 
                  type="text" required placeholder="Ej: 9023849102"
                  className="w-full bg-[var(--color-surface-elevated)] border border-zinc-850 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-[var(--color-primary)] font-mono"
                  value={pagoComprobante} onChange={e => setPagoComprobante(e.target.value)}
                />
                <p className="text-[10px] text-[var(--color-text-muted)] mt-1">Este número quedará vinculado a la DIAN como soporte de pago.</p>
              </div>

              <div>
                <label className="block text-xs font-bold text-zinc-300 mb-1">Enlace Soporte (Drive / Nube) - Opcional</label>
                <input 
                  type="url" placeholder="https://drive.google.com/..."
                  className="w-full bg-[var(--color-surface-elevated)] border border-zinc-850 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-[var(--color-primary)] text-xs"
                  value={pagoLink} onChange={e => setPagoLink(e.target.value)}
                />
              </div>
              ...
              <div className="mt-4 flex gap-3">
                <button 
                  type="button" 
                  onClick={() => setModalPagoId(null)}
                  className="flex-1 py-3 rounded-xl border border-zinc-800 text-white hover:bg-white/5 transition-all text-xs font-bold"
                >
                  Cancelar
                </button>
                <button 
                  type="submit"
                  className="flex-1 bg-[var(--color-primary)] hover:bg-[var(--color-primary-dark)] text-black font-extrabold py-3 rounded-xl transition-all text-xs flex justify-center items-center gap-2"
                >
                  <CheckSquare className="w-4 h-4" /> Confirmar Pago
                </button>
              </div>
            </form>
          </div>
